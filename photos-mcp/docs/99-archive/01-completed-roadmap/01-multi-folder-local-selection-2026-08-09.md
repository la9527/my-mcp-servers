...
- 상세 미리보기 높이는 inspector 너비뿐 아니라 실제 scroll viewport 높이도 함께 사용한다. 최소 창에서는 120px까지 줄여 파일명, 촬영 시각, 용량, 해상도와 핵심 노출 요약이 작업 설정 위에 보이도록 한다.
...
- 최소 창에서 발견한 상세 미리보기 과점유를 수정한 뒤 다시 캡처했다. 수정본은 파일 정보와 노출 요약을 scroll 시작 화면에 온전히 표시했고, 선택 목록의 두 폴더 그룹과 고정 작업 설정도 겹치지 않았다.
