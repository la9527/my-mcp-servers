...
기존 앨범 또는 새 앨범 이름을 지정한다. 이미 Apple 사진 보관함에 있는 결과는 해당 사진 UUID를 앨범에 추가하며, 이미 앨범에 있는 항목은 영수증에서 별도로 구분한다.

Google Photos 또는 로컬 폴더에서 가져와 분석한 결과는 Apple 사진 UUID가 아니다. 이 경우 앱은 사용자가 승인한 파일만 Apple 사진 보관함으로 **가져온 뒤** 지정한 앨범에 넣는다. 원본 Google Photos·로컬 파일은 변경하거나 이동하지 않는다. 같은 파일이 이미 보관함에 있으면 Apple Photos의 중복 확인 결과가 영수증에 반영될 수 있다.
...
내보내기 계획이 파일·권한·대상 검증 단계에서 차단된 경우에는 실제 보관함 변경이 일어나지 않는다. 이 유형의 기록은 재시도를 막는 "재조정 필요" 상태로 남기지 않고, 원인을 수정한 뒤 같은 선택으로 새 계획을 만들 수 있다. 반대로 Apple Photos 자동화가 시작된 뒤의 시간 초과·부분 성공은 중복 추가를 막기 위해 영수증 기준 재확인을 유지한다.
