...
기존 대표 얼굴은 인물을 처음 확정할 때 선택된 한 장이 `representative_face_versions`의 active 항목으로 계속 유지됐다. 이후 더 선명하고 정면에 가까운 얼굴이 여러 장 확정돼도 최초 얼굴이 우선되어, 실제 운영 데이터에서는 품질 제외 얼굴과 정면 점수 `0.0`인 옆모습이 인물 카드에 남아 있었다.

대표 얼굴은 이제 삭제나 수동 재지정 없이 확정 얼굴 전체에서 매번 결정적으로 계산한다.

1. `auto_eligible`·`review_eligible` 얼굴을 품질 제외 얼굴보다 우선한다.
2. 정면 점수를 `0.90 이상`, `0.78 이상`, `0.65 이상`, 그 미만의 네 구간으로 나누어 옆모습보다 정면 얼굴을 우선한다.
3. 같은 정면 구간에서는 정면성 42%, 선명도 22%, detector 신뢰도 14%, 얼굴 crop 크기 12%, 노출 10%를 합산하고 잘림 비율을 감점한다.
4. 품질 기준을 통과한 얼굴이 없을 때만 품질 제외 얼굴을 fallback으로 사용한다.
5. 기존 active 대표 얼굴 표시는 모든 품질 값이 같은 경우의 안정적인 마지막 tie-breaker로만 사용한다.

이 projection은 Mac과 Android가 함께 사용하는 identity 저장소에 구현했다. macOS 화면도 최근 작업 메모리의 첫 얼굴보다 저장소가 고른 품질 대표 얼굴을 먼저 사용한다. 따라서 새 분석에서 더 좋은 확정 얼굴이 추가되면 양쪽 앱의 대표 사진이 함께 개선되며, 이름·얼굴 연결 정보나 원본 사진은 변경하지 않는다.

동일한 정렬을 반복 등장한 새 인물의 3장 근거 crop에도 적용했다. 따라서 새 인물 확인 팝업과 Android 확인 화면도 날짜나 등록 순서가 아니라 정면·선명도 품질이 높은 사진부터 보여준다.

운영 데이터의 기존 3명을 새 기준으로 재평가한 결과, 선택된 대표 crop의 정면 점수는 각각 약 `0.998`, `0.988`, `0.795`였다. 기존의 품질 제외 얼굴 2장과 정면 점수 `0.0`인 옆모습 대신 실제 정면 crop이 선택되는 것을 파일과 앱 화면에서 확인했다.
