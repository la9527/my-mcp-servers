...
중앙 격자는 화면 폭과 밀도 설정을 함께 사용해 3~6열 범위에서 결정한다. 한 장 보기와 격자 모두 좌우 키로 포커스를 이동하고 Enter·Space로 분류 대상을 토글한다. 한 장 보기는 전체 화면 뷰어와 같은 `NSScrollView` 기반 확대 canvas를 사용해 지점 중심 확대, drag 이동, 화면 맞춤과 100% 복귀를 제공한다.
...
- double-click과 trackpad smart zoom은 선택한 사진 지점을 중심으로 확대하고 다시 실행하면 화면 맞춤으로 복귀한다.
- pinch와 `Command` + scroll은 제스처·포인터 위치를 기준으로 확대·축소한다. 일반 scroll은 사진 이동이다.
- 확대된 사진은 ImageKit 좌표 변환을 사용하는 read-only drag 이동을 제공하며, 확대 범위는 제한하고 “화면에 맞춤”과 “실제 크기” 복귀를 제공한다.
- 좌우 이동과 현재 위치를 표시한다.
